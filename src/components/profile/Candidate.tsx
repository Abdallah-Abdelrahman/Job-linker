 ...
 */
const actionCreator = (payload: A['payload'], type?: A['type']) => ({
  payload,
  type,
});
...
function Candidate({ data, as }: T.CandidateProp) {
  ...
  const [update_candid, { isLoading: isLoading_MCANDID }] =
    useUpdateCurrentCandidateMutation();
  const [upload, { isLoading: isLoading_MUPLOAD }] =
    useUploadProfileImageMutation();
  const handleUpdate = () => {
  ...
  return (
    <Box className="grid grid-cols-4 gap-6 container mt-4 mx-auto sm:grid-cols-12">
      {/* Contact & skills */}
      <Box className="relative  col-span-4 bg-white flex p-6 flex-col items-center gap-2 rounded-md shadow-md sm:col-span-4 sm:max-h-[900px] sm:overflow-auto">
        {as ? null : (
          <Button
        ...
        {isEditing && !as ? (
          <FormControl size="lg">
            <Input
              value={state.name}
              onChange={(e) =>
                dispatch(actionCreator({ name: e.target.value }))
              }
            />
          </FormControl>
        ) : (
        ...
        {isEditing && !as ? (
          <FormControl size="lg">
            <Select
              value={state.major}
              onChange={(e) =>
                dispatch(actionCreator({ major: e.target.value }))
              }
            >
              {college_majors.map((m, idx) => (
                <option key={idx} value={m} children={m} />
              ))}
            </Select>
          </FormControl>
        ) : (
          <Text className="tracking-wide">
            {renderedData?.candidate?.major.name}
          </Text>
        )}
        <hr className="w-full" />
        <Stack className="w-full mt-2 space-y-6">
          ...
          <Box as="section" className="flex flex-col gap-4">
            ...
            <List
              as="ul"
              className="flex flex-wrap gap-4 max-h-40 overflow-auto"
            >
          ...
          <List as="ul" className="flex flex-col gap-4">
            {renderedData?.candidate?.education.map((ed, idx, arr) => (
              <Fragment key={idx}>
                ...
                {idx !== arr.length - 1 && <Divider colorScheme="blue" />}
              </Fragment>
            ))}
          </List>
        </Box>
...
type ContactProps = {
  data: Record<
    'address' | 'github' | 'linkedin' | 'whatsapp' | 'phone',
    string
  >;
          ...
          <Box key={k} className="flex gap-3">
            {isEditing ? (
              <>
                <InputGroup>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      dispatch(
                        actionCreator({ [k]: e.target.value }, 'contact_info'),
                      )
                    }
                  />
                </InputGroup>
              ...
              <>
                <MyIcon href={`/sprite.svg#${k}`} className="w-5 h-5" />
                {k == 'linkedin' || k == 'github' ? (
                  <a href={v} target="_blank" className="text-sky-500">
                    {k}
                  </a>
                ) : (
                  <Text>{v}</Text>
