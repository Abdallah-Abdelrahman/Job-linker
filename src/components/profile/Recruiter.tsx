...
function Recruiter({ data }: T.RecruiterProp) {
  ...
  const [uploadProfileImage, { isLoading: isLoading_MUPLOAD }] =
    useUploadProfileImageMutation();
      ...
      <Box className="relative col-span-4 bg-white flex p-6 flex-col items-center gap-2 rounded-md shadow-md sm:col-span-4">
        ...
        {match && (
          <List className="w-full mt-4 capitalize">
            <Heading size="base" mb="2">
              applied candidates
            </Heading>
            {appliedCandids.length <= 0 && (
              <Text
                className="text-slate-400"
                children="no canddiate has applied yet for this job!"
              />
            )}
            {appliedCandids.map((cand, idx) => {
              ...
              return (
                <ListItem
                  key={idx}
                  className="relative p-2 space-y-2 border shadow-md rounded-md "
                >
                  <Flex gap="2">
                    <Box className="bg-slate-100 p-2 rounded-full">
                      <MyIcon
                        href="/sprite.svg#user"
                  ...
                  <Box
                    className={`p-2 absolute top-2 right-2 ${bg[cand.application_status]} rounded-full`}
                  >
                    <Text
                      className={`${clr[cand.application_status]}`}
                      children={cand.application_status}
                    />
                  </Box>
                </ListItem>
              );
