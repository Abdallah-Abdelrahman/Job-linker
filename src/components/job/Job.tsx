...
type Context = {
  setAppliedCandidates?: React.Dispatch<React.SetStateAction<never[]>>;
};
  ...
  const [state, dispatch] = useReducer(
    (prevState, newState) => {
      ...
      if (newState.type === 'init') {
        const { created_at, applied_candidates, id, is_open, ...rest } =
          newState.payload;
        return rest;
      }
    ...
    apply({ job_id })
      ...
      .then((_) =>
        toast({
          title: 'application',
          description:
            'your application has been sent successfuly, check your email',
          status: 'success',
          isClosable: true,
          position: 'top',
          variant: 'left-accent',
          ...
          {isEditing ? (
            ...
            <Stack className="!mt-12">
              <InputGroup>
                <Input
                  ...
                  onChange={(e) => dispatch({ job_title: e.target.value })}
                />
              </InputGroup>
              <InputGroup>
              ...
              <InputGroup>
                ...
                <Input
                  type="datetime-local"
                  value={new Date(state.application_deadline)
                    .toISOString()
                    .slice(0, 16)}
                  onChange={(e) =>
                    dispatch({ application_deadline: e.target.value })
                  }
                />
              </InputGroup>
              <UpdateOrCancel
            ...
            <>
              ...
              <Box>
                ...
                <Text className="" children={job.data.application_deadline} />
              </Box>
            </>
          )}
        </Box>
